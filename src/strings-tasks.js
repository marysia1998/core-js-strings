...
function getStringLength(value) {
  return typeof value === 'string' && value.length ? value.length : 0;
}
...
function removeFirstOccurrences(str, value) {
  ...
  const first = str.substring(0, indexOfFirst);
  const second = str.substring(indexOfFirst + value.length, str.length);
  return indexOfFirst !== -1 ? first + second : str;
}
...
function formatTime(minutes, seconds) {
  ...
  return newMinutes + separate + newSeconds;
}
...
function countVowels(str) {
  const result = str.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '');
  const reverceStr = newStr.split('').reverse().join('');
  return reverceStr === newStr;
}
...
function reverseWords(str) {
  ...
  const newArray = wordsArray.map((word) => word.split('').reverse().join(''));
...
function invertCase(str) {
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    const lowWord = str[i].toLowerCase();
    if (lowWord === str[i]) {
